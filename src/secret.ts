...
class Secret {
    ...
    constructor() {
        ...
        this.kts = [
            240, 229, 105, 174, 191, 220, 191, 138, 26, 69, 232, 190, 125, 166,
            115, 184, 222, 143, 231, 196, 69, 218, 134, 196, 155, 100, 139, 20,
            106, 180, 241, 170, 56, 1, 53, 158, 38, 105, 44, 134, 0, 107, 79,
            165, 54, 52, 98, 166, 42, 150, 104, 24, 242, 74, 253, 189, 107, 151,
            143, 77, 143, 137, 19, 183, 108, 142, 147, 237, 14, 13, 72, 62, 215,
            47, 136, 216, 254, 254, 126, 134, 80, 149, 79, 209, 235, 131, 38,
            52, 219, 102, 123, 156, 126, 157, 122, 129, 50, 234, 182, 51, 222,
            58, 169, 89, 52, 102, 59, 170, 186, 129, 96, 72, 185, 213, 129, 156,
            248, 108, 132, 119, 255, 84, 120, 38, 95, 190, 232, 30, 54, 159, 52,
            128, 92, 69, 44, 155, 118, 213, 27, 143, 204, 195, 184, 245,
        ];
        ...
        if (key != null) {
            return (() => {
                ...
                for (
                    ...
                    results.push(
                        ((key[i] + this.kts[length * i]) & 0xff) ^
                            this.kts[length * (length - 1 - i)]
                    );
                }
                return results;
            })();
    ...
    asymEncode(src: string | any[], srclen: number) {
        ...
        for (
            ...
            ret += this.rsa.encrypt(
                this.bytesToString(
                    src.slice(i * m, Math.min((i + 1) * m, srclen))
                )
            );
        }
    ...
    encode(str: any, timestamp: any) {
        const key = this.stringToBytes(
            SparkMD5.hash(`!@###@#${timestamp}DFDR@#@#`)
        );
